...
'''
class XMLCompiler(ce.CompilationEngine):
    ...
    def compile_class(self):
        ...
        while self.stream.token_type() == tokenizer.KEYWORD and self.stream.keyword() in ce.CLASS_VARS:
            self.compile_class_var_dec()

        while self.stream.token_type() == tokenizer.KEYWORD and self.stream.keyword() in ce.SUBROUTINE_TYPES:
    ...
    def compile_class_var_dec(self):
        ...
        '''
        class_var_root = ET.SubElement(self.root, ce.CLASS_VAR_DEC)
        ...
        while self.stream.symbol() == ce.COMMA:
    ...
    def compile_subroutine(self):
        ...
        '''
        subroutine_dec = ET.SubElement(self.root, ce.SUBROUTINE_DEC)
        ...
        subroutine_body = ET.SubElement(subroutine_dec, ce.SUBROUTINE_BODY)
        self.add_terminal(subroutine_body, self.stream.symbol())
        while self.stream.token_type() == tokenizer.KEYWORD and self.stream.keyword() == ce.VAR:
    ...
    def compile_parameter_list(self, root):
        ...
        '''
        parameter_list_root = ET.SubElement(root, ce.PARAMETER_LIST)
        ...
        while self.stream.token_type() == tokenizer.SYMBOL and self.stream.symbol() == ce.COMMA:
    ...
    def compile_var_dec(self, root):
        ...
        '''
        var_dec_root = ET.SubElement(root, ce.VAR_DEC)
    ...
    def compile_statements(self, root):
        ...
        '''
        statements_root = ET.SubElement(root, ce.STATEMENTS)
    ...
    def compile_do(self, root):
        ...
        '''
        do_root = ET.SubElement(root, ce.DO)
    ...
    def compile_let(self, root):
        ...
        '''
        let_root = ET.SubElement(root, ce.LET)
        self.add_terminal(let_root, self.stream.keyword())
        self.add_terminal(let_root, self.stream.identifier())
        if self.stream.token_type() == tokenizer.SYMBOL and self.stream.symbol() == ce.OPEN_BRACKET:
    ...
    def compile_while(self, root):
        ...
        '''
        while_root = ET.SubElement(root, ce.WHILE)
    ...
    def compile_return(self, root):
        ...
        '''
        return_root = ET.SubElement(root, ce.RETURN)
    ...
    def compile_if(self, root):
        ...
        '''
        if_root = ET.SubElement(root, ce.IF)
    ...
    def compile_expression(self, root):
        ...
        '''
        expression_root = ET.SubElement(root, ce.EXPRESSION)
        self.compile_term(expression_root)
        while self.stream.token_type() == tokenizer.SYMBOL and self.stream.symbol() in ce.OPS:
    ...
    def compile_term(self, root):
        ...
        '''
        term_root = ET.SubElement(root, ce.TERM)
        ...
        elif token_type == tokenizer.KEYWORD and self.stream.keyword() in ce.KEYWORD_CONSTANTS:
            self.add_terminal(term_root, self.stream.keyword())
        elif token_type == tokenizer.IDENTIFIER:
            if self.stream.peek() == ce.OPEN_BRACKET:
                ...
                self.add_terminal(term_root, self.stream.symbol())
            elif self.stream.peek() == ce.OPEN_PAREN or self.stream.peek() == ce.PERIOD:
                self.compile_subroutine_call(term_root)
            else:
                self.add_terminal(term_root, self.stream.identifier())
        elif token_type == tokenizer.SYMBOL and self.stream.symbol() == ce.OPEN_PAREN:
            self.add_terminal(term_root, self.stream.symbol())
            self.compile_expression(term_root)
            self.add_terminal(term_root, self.stream.symbol())
        elif token_type == tokenizer.SYMBOL and self.stream.symbol() in ce.UNARY_OPS:
            self.add_terminal(term_root, self.stream.symbol())
    ...
    def compile_expression_list(self, root):
        ...
        '''
        expression_list_root = ET.SubElement(root, ce.EXPRESSION_LIST)
        if self.stream.token_type() == tokenizer.SYMBOL and self.stream.symbol() == ce.CLOSE_PAREN:
            return
    ...
    def compile_subroutine_call(self, root):
        ...
        if self.stream.symbol() == ce.PERIOD:
